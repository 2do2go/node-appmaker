...
exports.requirejsOptimize = function(config) {
	...
	requirejs.optimize(config, function() {
		...
		if (config.optimizeParallel) {
			var modulesSubDir = modulesDir.replace(baseUrl, '');
			if (!modulesSubDir) throw new Error('Can`t determine modules sub dir');
			exports.jsOptimizeParallel({dir: path.join(config.dir, modulesSubDir)}, function(err) {
				if (err) throw (err);
			});
		}
	}, function(err) {
