...
exports.requirejsOptimize = function(config, callback) {
	...
	if (config.modules) {
		...
		_(config.modules).each(function(module) {
			...
			var modulesNames = exports.getModulesNames(module.path, config);

			_(modulesNames).each(function(moduleName) {
				module = _({name: moduleName}).defaults(module);
				modules.push(module);
			});
		});
...
exports.getModulesNames = function(modulePath, config) {
	...
	if (fs.statSync(path.resolve(modulePath)).isDirectory()) {
		shell.find(modulePath).filter(function(file) {
			return (
				file.match(/\.js$/) && (
			...
			names.push(modulePathToName(modulePath));
		});
	} else {
		names.push(modulePathToName(modulePath));
	}
