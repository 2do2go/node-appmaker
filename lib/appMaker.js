...
exports.requirejsOptimize = function(config, callback) {
	...
	printHeader('normalize ' + config.modules.join(', ') + ' to paths');
	var modules = [];
	_(config.modules).each(function(modulePath) {
		if (fs.statSync(path.resolve(modulePath)).isDirectory()) {
			shell.find(modulePath).filter(function(file) {
				...
				modules.push(modulePathToModule(modulePath));
			})
		} else {
			modules.push(modulePathToModule(modulePath));
		}
	});
	function modulePathToModule(module) {
		return {name: module.replace(config.baseUrl, '').replace('.js', '')};
	}
